...
def beh_analysis_no_plot(session_name, rev_for_flag=0, make_fig_flag=0, 
                              t_max=10, time_max=61000, time_bins=6, simple_flag=True):
    """
    ...
    lick_lat = beh_session_data.loc[response_inds, "respondTime"].values - beh_session_data.loc[response_inds, "CSon"].values
    ...
    # CS+ and CS- trials
    csplus_inds = [i for i, x in enumerate(beh_session_data["trialType"]) if x == "CSplus"]
    csminus_inds = [i for i, x in enumerate(beh_session_data["trialType"]) if x == "CSminus"]
